...
typedef struct
{
   ...
   int       N;
   int       DEG;
   rlu_relaxed_obj_t* p_new_counter;
...
//Global Variables
int min = PR_INT_MAX;    //For local mins
int min_index = 0;
...
rlu_thread_data_t rlu_thread_data[1024];
...
void* do_work(void* args)
{
   ...
   rlu_relaxed_obj_t* p_new_counter     = arg->p_new_counter;

   int node = 0;

   //yunduz rlu
   rlu_thread_data_t* self = arg->rlu_td;
   RLU_THREAD_INIT(self);

   // int *local_counter = (int*) RLU_RELAXED_DEREF_TO_WRITE(self, p_new_counter);
   // printf("yunduz: local counter = %d\n", *local_counter);
   // (*local_counter)++;
   // (*local_counter)++;
   // printf("yunduz: local counter = %d\n", *local_counter);
   // int combined_counter = RLU_RELAXED_GET_REDUCTION_COUNTER_VAL_INT(p_new_counter, 0);
   // printf("yunduz: combined counter = %d\n", combined_counter);
   ...
   while(node<N)
   {
      // printf("i = %d\n", i);
      // ++i;
      test_node = RLU_RELAXED_GET_REDUCTION_COUNTER_VAL_INT(p_new_counter, 0);
      // printf("yunduz: test_node = %d\n", test_node);
      int *local_counter = (int*) RLU_RELAXED_DEREF_TO_WRITE(self, p_new_counter);
      (*local_counter)++;
...
int main(int argc, char** argv)
{
   ...
   rlu_relaxed_obj_t *p_new_counter = (rlu_relaxed_obj_t *)RLU_RELAXED_ALLOC(sizeof(int), 1);
   if (p_new_counter == NULL){
   ...
   for(int j = 0; j < P1; j++) {
      ...
      thread_arg[j].rlu_td     = &rlu_thread_data[j];
      thread_arg[j].p_new_counter = p_new_counter;
   }
      ...
      int   N)
{
   for(int i = 0; i < N+1; i++)
   {
      D[i] = PR_INT_MAX;  //all distances infinite
      Q[i] = 1;
   }
...
void relax(int u, int i, volatile int* D, int** W, int** W_index, int N)
{
   if((D[W_index[u][i]] > (D[u] + W[u][i]) && (W_index[u][i]!=-1 && W_index[u][i]<N && W[u][i]!=PR_INT_MAX)))
