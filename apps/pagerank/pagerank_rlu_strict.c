...
int nodecount = 0;
thread_arg_t thread_arg[50];//[1024];    //MAX threads
      ...
      for(v=i_start;v<i_stop;v++)
      {
         ...
         if(exist[v]==1)   //if vertex exists
         {
            ...
            for(j=0;j<test[v];j++)
            {
               ...
               tmp = tmp + (d*(RLU_STRICT_GET_VAL_DOUBLE(self, PR[W_index[v][j]]))/outlinks[W_index[v][j]]);
            }
         }
				 // if(pgtmp[v]>=1.0)
					//  pgtmp[v] = 1.0;
         ...
         RLU_STRICT_SET_VAL_DOUBLE(self, PR[v], tmp);
      }
      // //printf("\n Ranks done");
...
{
   // double** PR;
   // if(posix_memalign((void**) &PR, 64, num_nodes * sizeof(double*)))
   // {
...
int main(int argc, char** argv)
{
   ...
   RLU_THREAD_FINISH(&(thread_arg[0].rlu_td));

   //Join threads
   for(j = 1; j < P; j++) { //mul = mul*2;
