...
void* do_work(void* args)
{
   ...
   //yunduz
   //hack fix bug where stop is greater than the number of vertices
   if(stop > largest)
   {
      stop = largest;
   }
   ...
   for(v=start;v<stop;v++)
   {
      if(exist[v]==1)
      {
         for(int i = 0; i < edges[v]; i++)
         {
            ...
            // int b = __sync_fetch_and_add(&global_d,1);
            Q[W_index[v][i]] = 0;
         }
      }
   }
...
int main(int argc, char** argv)
{
  ...
  while ((ca = getopt(argc, argv, "s:::t::m::")) != -1)
   ...
   // do we need to run it multiple times aka it's an experiment
   int decrease_threads=0;

   if(((select==1 || select==2) && argc > 4) || (select==0 && argc > 5))
   {
      decrease_threads=1;
   }   
   ...
   if(select==0)
   {
      N = atoi(argv[3]);
      DEG = atoi(argv[4]);
      printf("Graph with Parameters: N:%d DEG:%d\n",N,DEG);
   }
   if (DEG > N)
   {
   ...
   // printf("repeat:%d\tdecrease_threads:%d\n", repeat, decrease_threads);
   for(;num_threads<max_num_threads+1; num_threads++)
   {
      for(int x=0; x < repeat; ++x)
      {
         ...
         Total_Tri = 0;
         memset(Total_tid, 0, sizeof(Total_tid));
         ...
         printf("triangle_counting\t%lf\t%lld\t%d\n", accum, count, P);
