...
int initialize_single_source(int* Q, int source, int N);
  ...
  RLU_THREAD_INIT(self);
   ...
   for(v=start;v<stop;v++)
   {
      if(exist[v]==1)
      {
         for(int i = 0; i < edges[v]; i++)
         {
            ...
            Q[neighbor] = 0;
            //yunduz rlu
            volatile int* local_edge_counter = (int*) RLU_RELAXED_DEREF_TO_WRITE(self, D);
            local_edge_counter[neighbor]++;
         }
      }
   }
   ...
   for(v=start;v<stop;v++)
   {
      if(exist[v]==1)
      {
        // float temp = D[v];
        //yunduz rlu
        float temp = RLU_RELAXED_GET_REDUCTION_COUNTER_VAL_INT(D, v);
...
rlu_relaxed_obj_t *create_rlu_D(int num_vertices)
{
  rlu_relaxed_obj_t *p_new_counter = (rlu_relaxed_obj_t *)RLU_RELAXED_ALLOC(sizeof(int), num_vertices);
  if (p_new_counter == NULL){
      printf("out of memory\n");
      exit(1);
  }

  return p_new_counter;
}
...
int main(int argc, char** argv)
{
  RLU_INIT(RLU_TYPE_FINE_GRAINED, 1);
   ...
   // yunduz rlu
   rlu_relaxed_obj_t *D = create_rlu_D(N);
   ...
   if(select!=2)
   {
     initialize_single_source(Q, 0, largest);
   }
   else
     initialize_single_source(Q, 0, largest);
...
int initialize_single_source(int*  Q,
      int   source,
      int   N)
{
   for(int i = 0; i < N; i++)
   {
      // D[i] = 0; //yunduz rlu, D is initialized to 0 when rlu-relaxed alloced
      Q[i] = 1;
   }
